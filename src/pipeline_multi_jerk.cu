...
private:
  ...
  JerkPlan& jerk_plan;   // PVP .. added corresponding jerkplan class in utils.hpp
  unsigned int size;
  int device;
...
public:
  ...
  Worker(DispersionTrials<DedispOutputType>& trials, DMDispenser& manager, 
	 AccelerationPlan& acc_plan, JerkPlan& jerk_plan, CmdLineOptions& args, unsigned int size, int device)    // PVP added jerk plan
    :trials(trials),manager(manager),acc_plan(acc_plan),jerk_plan(jerk_plan),args(args),size(size),device(device){}
  ...
  void start(void)
  {
    ...
    TimeDomainResampler resampler;  // PVP added resampleIII in resampler.hpp for acc+jerk
    ...
    std::vector<float> jerk_list;
    HarmonicDistiller harm_finder(args.freq_tol,args.max_harm,false);
    AccelerationDistiller acc_still(tobs,args.freq_tol,true);
    //AccJerkDistiller acc_jerk_still(tobs,args.freq_tol,true); #// PVP added AccJerkDistiller class to distiller.hpp but commented for now due to wrong formula
    float mean,std,rms;
    float padding_mean;
    int ii;
	...
	std::cout << "Copying DM trial to device (DM: " << tim.get_dm() << ")"<< std::endl;
      ...
      if (args.verbose)
	    std::cout << "Generating jerk list" << std::endl;
      jerk_plan.generate_jerk_list(tim.get_dm(),jerk_list); // PVP added 
      
      if (args.verbose)
	    std::cout << "Searching "<< acc_list.size()<< " acceleration trials for DM "<< tim.get_dm() << std::endl;

      if (args.verbose)
	    std::cout << "Searching "<< jerk_list.size()<< " jerk trials for DM "<< tim.get_dm() << std::endl; // PVP added
      ...
      CandidateCollection acc_jerk_trial_cands;    //!! Check this
      PUSH_NVTX_RANGE("Acceleration-Loop",1)


      
      for (int jj=0;jj<acc_list.size();jj++){               // PVP added another jerk loop here
          for (int kk=0;kk<jerk_list.size();kk++){              
                if (args.verbose)
                ...
                SpectrumCandidates trial_cands(tim.get_dm(),ii,acc_list[jj],jerk_list[kk]); //PVP changed SpectrumCandidates class in candidates.hpp to taker jerk too
                ...
                if (args.verbose)
                  ...
                  acc_jerk_trial_cands.append(harm_finder.distill(trial_cands.cands));
              }
          }    
	  POP_NVTX_RANGE
      if (args.verbose)
	    std::cout << "Distilling accelerations" << std::endl;
      dm_trial_cands.append(acc_still.distill(acc_jerk_trial_cands.cands)); /// PVP added Acc+Jerk distilling (!!! Not too sure on the way this is done... so sticking to acc only)
    }
	POP_NVTX_RANGE
...
//  Utils::dump_host_buffer<unsigned int>(data_ptr,trials.get_nsamps(),"dedispersed_timeseries_new");
  ...
  JerkPlan jerk_plan(args.jerk_start, args.jerk_end, args.jerk_tol,    //// PVP
			    args.jerk_pulse_width, size, filobj.get_tsamp(),
			    filobj.get_cfreq(), filobj.get_foff()); 
  ...
  for (int ii=0;ii<nthreads;ii++){
    workers[ii] = (new Worker(trials,dispenser,acc_plan,jerk_plan,args,size,ii));   ///// PVP
  ...
  OutputFileWriter stats; // Modified class to take in jerk parameters
  stats.add_misc_info();
  ...
  std::vector<float> jerk_list; // PVP added jerk
  jerk_plan.generate_jerk_list(0.0,jerk_list); 
  stats.add_jerk_list(jerk_list);
